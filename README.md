...
| Validation for the correct input | Repeatedly requests input from the user until valid input is provided. If the input type is neither str, int, nor float, an error message is shown. This prevents the user from using an unsupported type and alerts them of the issue. | def get_valid_input(prompt, input_type=str, valid_range=None) |
