...
|find stock symbol | The feature provides a list of stock symbols which has to be selected form the user. This symbol is relevant to creat the correct API request url for further functions.| def find_stock_symbol() |
|provide updated data | The feature updates the current price automatically. It pulls the relevant symbol data and requets the relevant infomration from the Alpha Vatange API. Then it updates the cells in the google spreadsheet. | def provide_updated_data() |
...
| API request logic | This function is used to deal with API requests. It wraps an API call with retry logic. It will retry up to 5 times in case of error or any other recoverable error. It uses a retry mechanism with exponential backoff, making it resilient to temporary issues like server errors (e.g., HTTP 500 or 503). It is used to wrap critical API calls and ensures that these operations are retried if a error occurs. For example, when adding or updating stock information in Google Sheets: "api_call_with_retry(stock_portfolio.update_cell, 2, last_column + 1, new_stock_name)". This ensures that if there’s an issue (like a temporary service disruption), the code will retry the operation before failing, preventing the program from crashing due to short-term API issues. | def api_call_with_retry(api_method, *args, **kwargs) |
| Validation for the correct input | Repeatedly requests input from the user until valid input is provided. If the input type is neither str, int, nor float, or a non vlaid range is used an error message is shown. This prevents the user from using an unsupported type and alerts them of the issue. The range is from 0 to max. 500.000 since a negative value make sno sense for number of shares or stock price. In addition, currently the most valueable stock price is 429.500$ (Berkshire Hathaway). | def get_valid_input(prompt, input_type=str, valid_range=0, 500000) |
...
| 13 | api_method(*args, **kwargs) | https://stackoverflow.com/questions/3394835/use-of-args-and-kwargs, https://www.youtube.com/watch?v=4jBJhCaNrWU, https://www.youtube.com/watch?v=Vh__2V2tXUMhttps://www.youtube.com/watch?v=4jBJhCaNrWU, https://www.youtube.com/watch?v=4jBJhCaNrWU 
| 14 | Fixation of Google API issue | Code suggestion by Mentor Chris Quinn and Google documentation | https://googleapis.github.io/google-api-python-client/docs/epy/googleapiclient.errors-module.html, https://googleapis.dev/python/google-api-core/latest/retry.html |
| 15 | Retry Logic in APIs | Idea and some Code suggestion by Mentor Chris Quinn, docs-python, google | https://docs.python.org/3/tutorial/errors.html, https://en.wikipedia.org/wiki/Exponential_backoff, https://pypi.org/project/retrying/, https://cloud.google.com/iam/docs/retry-strategy, https://cloud.google.com/memorystore/docs/valkey/exponential-backoff |
| 16 | hide API-Key via Heroku | Advice by Mentor Chris Quinn ||
